...
source: "https://mp.weixin.qq.com/s/xjxbwZBgz4a4Fy7hs6MdaA?scene=1&click_id=9"
author:
  - "[[Duran]]"
published:
created: 2026-04-13
description: "OpenClaw 最新版本让你可以直接控制真实 Chrome 浏览器，保留所有登录状态，无需安装任何插件"
tags:
  - "clippings"
---
原创 Duran *2026年3月15日 17:09*

> **一句话总结** ：OpenClaw 最新版本让你可以直接控制真实 Chrome 浏览器，保留所有登录状态，无需安装任何扩展。
...
在浏览器中访问以下地址，应该能看到可调试的页面列表：

```js
http://localhost:9222/json
```

> ⚠️ **安全提示** ：Remote Debugging 功能默认只在本地回环地址 (127.0.0.1) 监听，不会暴露到外部网络。OpenClaw 通过本地连接与该服务通信。
...
```json
{  "browser": {    "profiles": {      "user": {        "type": "existing-session",        "cdpUrl": "http://127.0.0.1:9222"      },      "openclaw": {        "type": "managed"      }    },    "defaultProfile": "user"  }}
```
...
```bash
# 查看当前连接的浏览器状态openclaw browser status
# 使用 user profile 连接到当前 Chrome 会话openclaw browser snapshot --profile user
# 在特定标签页执行操作openclaw browser click "登录按钮" --profile useropenclaw browser type "input[name='search']" "OpenClaw" --profile user
```
...
```bash
# Chrome 地址栏访问：chrome://inspect/#remote-debugging，确保已启用openclaw browser snapshot --profile user
# AI 可以看到你的 Gmail 界面并执行操作：# "帮我把未读邮件标记为已读并归档"
```
...
```bash
# 1. 启用 Chrome Remote Debugging（一次设置）chrome://inspect/#remote-debugging → 启用
# 2. 直接使用openclaw browser --profile user
```
...
•OpenClaw 官方文档 - Browser <sup>[1]</sup>

•Chrome DevTools MCP 官方博客 <sup>[2]</sup>

•Chrome DevTools Protocol 文档 <sup>[3]</sup>

•OpenClaw GitHub Releases <sup>[4]</sup>

•Model Context Protocol (MCP) 规范 <sup>[5]</sup>

**本文撰写于 2026年3月15日，基于 OpenClaw 2026.3.13 版本和 Chrome DevTools MCP 官方文档**

### References

`[1]` OpenClaw 官方文档 - Browser: *https://docs.openclaw.ai/tools/browser*  
`[2]` Chrome DevTools MCP 官方博客: *https://developer.chrome.com/blog/chrome-devtools-mcp-debug-your-browser-session*  
`[3]` Chrome DevTools Protocol 文档: *https://chromedevtools.github.io/devtools-protocol/*  
`[4]` OpenClaw GitHub Releases: *https://github.com/openclaw/openclaw/releases*  
