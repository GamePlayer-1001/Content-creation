...
pipeline.js: 内容流水线 API，承接任务、阶段、热点与遗留兼容接口。
pipeline-legacy-routes.js: 流水线兼容路由层，承接旧工作流接口并桥接 shared pipeline。
pipeline-route-support.js: 流水线路由辅助层，负责参数归一化、SSE、任务快照与输出文件辅助函数。
