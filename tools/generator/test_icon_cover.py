...
# -*- coding: utf-8 -*-
"""
[INPUT]: 依赖图标封面模板与本地输出目录
[OUTPUT]: 提供图标封面测试脚本
[POS]: tools/generator 验证脚本
...
def generate_icon_cover_html(title, subtitle, category, output_path):
    ...
    icon_config_file = project_root / 'config' / 'icons.json'
    ...
    # 读取模板
    template_file = project_root / 'assets' / 'templates' / 'cover_with_icons.html'
...
def test_all_categories():
    ...
    output_dir = project_root / 'output' / 'test_covers'
...
def generate_single_cover():
    ...
    project_root = Path(__file__).parent.parent.parent
    output_dir = project_root / 'output' / 'drafts' / 'test_publish'
    output_dir.mkdir(parents=True, exist_ok=True)
